...
window.submitSurvey = async (event, surveyId) => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      // Immediately update the activeIndex for the next survey and refresh
      if (!isLastSurvey) {
        localStorage.setItem("activeSurveyIndex", "0") // Always reset to the first remaining survey
      }

      // Always refresh surveys immediately without any delay
...
function generateQuestionInput(question, index, surveyColor = "#253074") {
  ...
  switch (question.type) {
    case "text":
      inputHtml = `<div class="input-field-container"><textarea name="q${index}" required class="response-input" rows="5" cols="150"></textarea></div>`
      break
    case "radio":
      inputHtml = `
